@VFG10X-602 @CashManagement @Regression
Feature: VFG10X-602 Cash Management _ Till IN _On or Above Security level

  Scenario: Cash Management _ Till IN _On or Above Security level

  #Login and add item to basket
    Given the sales assistant logs in to the application with "above" security level
    When the sales assistant clicks on Start Basket button

  #Store Operations Till in On or above security level
    ...
    And the sales assistant enters the quantity as 200 on first index
    ...
    And the sales assistant selects "Starting Bank" as reason code
    And the sales assistant clicks on proceed button
