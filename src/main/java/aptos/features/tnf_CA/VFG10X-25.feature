@VFG10X-25 @SaleTransaction @Regression @Card @Diners @SmokeTNF
Feature: VFG10X-25 Sale Transaction _Multiple Items with Line item & Transactional Discount _ Payment by Diners with PIN_ No customer
  ...
  #Login
    ...
    And the sales assistant enter discount amount as 100
    And the sales assistant clicks on Select a reason code button
    And the sales assistant selects "Damaged" as reason code
    And the sales assistant clicks on Apply button
    And the sales assistant lands on Basket Page
    ...
    And the sales assistant processes the payment with a "diners" card using a "insert" action, entering the pin "0046"
