@VFG10X-1638 @SFCC @PrerequisiteRequired

Feature: VFG10X-1638 SFCC_ Update a Dickies promotion without coupon

  Scenario: SFCC_ Update a Dickies promotion without coupon
    ...
    And the sales assistant enter the promotion ID "01082024"
    And the sales assistant select start date "10-Jan-2024"
    And the sales assistant select end date "11-Jan-2025"
    And sales assistant clicks on apply button in campaign page
    And sales assistant selects promotions from merchant tools
    And sales assistant enter the promotion ID "01082024" for promotions
