@VFG10X-2124 @StoreSelling @Regression @Prerequisite

  Feature: VFG10X-2124 Store Selling Functions _ Return of items from Multiple store in same transaction
  ...
  # Return from another store (completed using cash)
      ...
      And the sales assistant enters store number "300107" terminal "80" and transaction number "28"
      And the sales assistant return all items from the transaction
      And the sales assistant selects "Changed Mind" as reason code
      And the sales assistant lands on Basket Page
