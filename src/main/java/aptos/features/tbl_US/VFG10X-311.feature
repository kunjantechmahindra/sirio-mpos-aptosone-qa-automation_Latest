@VFG10X-311 @Customer @Regression
Feature: VFG10X-311 Customer _Customer creation with Email field empty

  Scenario: Customer _Customer creation with Email field empty
  ...
  #Create Customer with email as null
    ...
    And the sales assistant enter customer detail with "email" as "null"
    And the sales assistant clicks on Privacy Policy
    And the sales assistant clicks on Save button
    And the sales assistant validate if message is displayed as "email" is "Required"
